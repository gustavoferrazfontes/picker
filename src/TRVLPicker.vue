<template>
  <div :class="['trvl-picker-container', { relative: !vertical }]">
    ...
    <Portal :disabled="!hasPortal || !vertical" :target-el="`#${portal}`">
        ...
        >
          <header class="trvl-picker-header">
            ...
            <template v-if="vertical">
              ...
              <CloseButton class="trvl-picker-header-close" @click="close">
          ...
          <footer class="trvl-picker-footer">
            ...
            <aside class="trvl-picker-footer-buttons">
              <ClearButton
                ...
                :title="buttonClearTitle"
                @click="clearSelection"
              />
              <ApplyButton :title="buttonApplyTitle" @click="applySelection"/>
            </aside>
          </footer>
        </article>
      </Transition>
...
export default {
  ...
  methods: {
    ...
    handleClickOutside(e) {
      const isNotInside = element =>
        e.target !== element && !element.contains(e.target);

      if (
        isNotInside(this.$refs.picker) &&
        this.$refs.toggles.every(isNotInside)
      ) {
        this.close();
      }
    },
    ...
    setMonths() {
      ...
      if (this.breakpointTriple && width >= this.breakpointTriple)
        this.months = 3;
...
.trvl-picker {
  ...
  $month-width: calc(#{$day-size} * 7 + 16px * 2 + 2px);
