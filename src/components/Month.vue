<template>
  <div ref="month" :class="['trvl-picker-month', { vertical }]">
    <header class="trvl-picker-month-date">{{ dateString }}</header>

    <Week v-if="!vertical" :weeks="weeks"/>

    <div class="trvl-picker-month-days">
      <Spacer v-for="spacer in spacers.before" :key="`spacer-before-${spacer}`"/>
      ...
      <template v-if="!vertical">
        <Spacer v-for="spacer in spacers.after" :key="`spacer-after-${spacer}`"/>
      </template>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    dateString() {
      ...
      const month = this.startOfMonth.toLocaleString(this.monthLang, {
        month: "long"
      });
    ...
    endOfMonth() {
      return new Date(
        ...
        this.normalizedDate.getMonth() + 1,
        0
      );
    },
  ...
  mounted() {
    if (this.isSelectedMonth && this.vertical) {
      const pickerHeaderHeight = document.querySelector(".trvl-picker-header")
        .clientHeight;
